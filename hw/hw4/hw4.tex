...
{\singlespacing
\begin{algorithmic}
\Function{rangeSum}{$v,x_l,x_r$}
  \State $\mathcal{R} = \left[x_l,x_r\right]$
  \If{ $v \equiv Null$}
    \State {\bf return 0}
  \EndIf
  \If{ $v.key \in \mathcal{R}$ }
    \If{$v.parent.key \in \mathcal{R}$ {\bf and} $v.parent.key < v.key$}
      \State $sum_l = v.leftSum$
    \Else 
      \State $sum_l = $ \Call{rangeSum}{$v.left,x_l,x_r$}      
    \EndIf
    ...
    \Else 
      \State $sum_l = $ \Call{rangeSum}{$v.right,x_l,x_r$}      
    \EndIf
  ...
  \Else $\quad(v.key \notin \mathcal{R})$
    ...
    \If{$v.key > x_r$}
      \State $sum_l = $\Call{rangeSum}{$v.left,x_l,x_r$}
    \Else $\quad (v.key < x_l)$
      \State $sum_r = $\Call{rangeSum}{$v.right,x_l,x_r$}
    \EndIf
  \EndIf
  \State {\bf return} $sum_l + sum + sum_r$
\EndFunction
\end{algorithmic}
}
